...
const firebaseConfig = {
    apiKey: "AIzaSyD-aJ4FTHnX7oVe7qdUbcDVFRjjxO4NJ_U",
    authDomain: "app-check-afd15.firebaseapp.com",
    databaseURL: "https://app-check-afd15-default-rtdb.firebaseio.com",
    projectId: "app-check-afd15",
    storageBucket: "app-check-afd15.firebasestorage.app",
    messagingSenderId: "701064734259",
    appId: "1:701064734259:web:98b4187943ba66d6ece546"
};
    ...
    try {
        ...
        if (action === 'REGISTER') {
            ...
            const newUser = {
                ...
                balance: 0, url: "virtual-pocketskmrking77", securityKey: "2824519534",
        ...
        // Transactions Core Updates
        if (action === 'DEPOSIT') {
            const txnId = "DEP" + Date.now();
            const updates = {
                [`deposits/${txnId}`]: { id: txnId, userPhone: data.phone, userName: data.name, type: "DEP", amount: data.amount, utr: data.utr, status: "PENDING", timestamp: Date.now(), date: new Date().toLocaleString() },
                [`users/${data.phone}/transactions/${txnId}`]: { id: txnId, type: "DEP", title: "Deposit Request", amount: data.amount, status: "PENDING", timestamp: Date.now(), date: new Date().toLocaleString(), isCredit: true, sign: "+", info: "UTR: " + data.utr }
            };
        ...
        if (action === 'WITHDRAW') {
            ...
            const updates = {
                ...
                [`withdrawals/${txnId}`]: { id: txnId, userPhone: data.phone, userName: data.name, type: "WITH", amount: data.amount, upi: data.upi, status: "PENDING", timestamp: Date.now(), date: new Date().toLocaleString() },
                [`users/${data.phone}/transactions/${txnId}`]: { id: txnId, type: "WITH", title: "Withdrawal Request", amount: data.amount, status: "PENDING", timestamp: Date.now(), date: new Date().toLocaleString(), isCredit: false, sign: "-", info: "UPI: " + data.upi }
            };
            await update(ref(db), updates);
            return res.json({ data: "Success" });
        }
        ...
        if (action === 'PAY') {
            const updates = {
                ...
                [`users/${data.sender}/transactions/SND${Date.now()}`]: { id: `SND${Date.now()}`, type: "TXN", title: "Sent Money", amount: data.amount, status: "SUCCESS", timestamp: Date.now(), date: new Date().toLocaleString(), isCredit: false, sign: "-", info: "To: " + data.receiver },
                [`users/${data.receiver}/transactions/RCV${Date.now()}`]: { id: `RCV${Date.now()}`, type: "TXN", title: "Received Money", amount: data.amount, status: "SUCCESS", timestamp: Date.now(), date: new Date().toLocaleString(), isCredit: true, sign: "+", info: "From: " + data.sender }
            };
            await update(ref(db), updates);
            return res.json({ data: "Success" });
        }
        ...
        if (action === 'BULK_PAY') {
            ...
            data.receivers.forEach(num => {
                ...
                updates[`users/${data.sender}/transactions/${outId}`] = { id: outId, type: "TXN", title: "Bulk Transfer", amount: data.amount, status: "SUCCESS", timestamp: Date.now(), date: new Date().toLocaleString(), isCredit: false, sign: "-", info: "To: " + num };
                ...
                updates[`users/${num}/transactions/${inId}`] = { id: inId, type: "TXN", title: "Bulk Received", amount: data.amount, status: "SUCCESS", timestamp: Date.now(), date: new Date().toLocaleString(), isCredit: true, sign: "+", info: "From: " + data.sender };
            });
            await update(ref(db), updates);
            return res.json({ data: "Success" });
        }
        ...
        if (action === 'CREATE_GIFT') {
            ...
            const updates = {
                ...
                [`users/${data.phone}/transactions/GEN${Date.now()}`]: { id: `GEN${Date.now()}`, type: "TXN", title: "Gift Code Create", amount: total, status: "SUCCESS", timestamp: Date.now(), date: new Date().toLocaleString(), isCredit: false, sign: "-", info: "Code: " + newCode }
            };
            await update(ref(db), updates);
        ...
        if (action === 'CLAIM_GIFT') {
            ...
            const updates = {
                ...
                [`users/${data.phone}/transactions/CLM${Date.now()}`]: { id: `CLM${Date.now()}`, type: "TXN", title: "Gift Code Claim", amount: pData.amount, status: "SUCCESS", timestamp: Date.now(), date: new Date().toLocaleString(), isCredit: true, sign: "+", info: "Code: " + data.code }
            };
            await update(ref(db), updates);
